...
public class Lay : MonoBehaviour
{
    ...
    public void Construct(float x, int width, float unit)
    {
        ...
        {
            spline.InsertPointAt(i, new Vector3(x+i*unit, 1));
            spline.SetTangentMode(i, ShapeTangentMode.Continuous);
            spline.SetLeftTangent(i, new Vector3(-1, 0, 0));
            spline.SetRightTangent(i, new Vector3(1, 0, 0));
        }
        spline.InsertPointAt(width+1, new Vector3(x + width * unit, -20));
        ...
        spline.InsertPointAt(width+2, new Vector3(x, -20));
